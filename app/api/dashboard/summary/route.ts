...
export async function GET(req: Request) {
  try {
    ...
    if (selectedSalesId) {
      salesSubQuery = `
        SELECT branch_id, amount AS sales_total 
        FROM sales_expenses 
        WHERE id = ${db.escape(selectedSalesId)}
      `;
    }

    // 1. UPDATE branch table dynamically based on active selection
    await db.query(`
      UPDATE branch b
      LEFT JOIN (
    ...
    branches.forEach((b: any) => {
      ...
      b.total_expenses = b.salary_expenses + b.sales_expenses + b.other_expenses;
