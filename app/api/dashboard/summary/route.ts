...
export async function GET(req: NextRequest) {
  try {
    ...
    // Fetch all available Sales details safely with fallback columns
    ...
    // Fetch all available Capital details safely with fallback columns
    ...
    return NextResponse.json({
      ...
      sales: Array.isArray(salesRows) ? salesRows.map((r: any) => ({
        id: r.id,
        name: r.personName || r.person_name || r.description || `Sales Entry #${r.id}`,
