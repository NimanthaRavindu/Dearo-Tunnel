...
export default function DieselExpensesPage() {
  ...
  const totalUsedAmountFromList = useMemo(() => expenses.reduce((sum, item) => sum + numberValue(item.payable), 0), [expenses]);
  const filteredDieselTotal = useMemo(() => filteredExpenses.reduce((sum, item) => sum + numberValue(item.diesel), 0), [filteredExpenses]);
  const filteredAmountTotal = useMemo(() => filteredExpenses.reduce((sum, item) => sum + numberValue(item.payable), 0), [filteredExpenses]);
...
function CalculationCard({ title, value, description, icon, color }: { title: string; value: string; description: string; icon: React.ReactNode; color: "blue" | "red"; }) {
  const styles = { blue: { border: "border-blue-500/20", background: "bg-blue-500/10", icon: "text-blue-400", value: "text-blue-400" }, red: { border: "border-red-500/20", background: "bg-red-500/10", icon: "text-red-400", value: "text-red-400" } };
  const selected = styles[color];
