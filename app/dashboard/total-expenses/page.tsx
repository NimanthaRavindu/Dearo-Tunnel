...
function TotalExpensesContent() {
  ...
  const totalExpenses = branches.reduce((total, branch) => {
    return (
      total +
  ...
  return (
    <main className="min-h-screen bg-slate-950 p-3 font-sans text-slate-100 sm:p-5 lg:p-6">
      <div className="mx-auto max-w-7xl space-y-4">
        <header className="flex flex-col justify-between gap-3 border-b border-slate-800/80 pb-3 sm:flex-row sm:items-center">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="min-w-[190px] rounded-xl border border-slate-800 bg-slate-900 px-3.5 py-1.5">
              <span className="mb-0.5 block text-[9px] uppercase tracking-wider text-emerald-400">
        ...
        <section className="overflow-hidden rounded-xl border border-slate-800/90 bg-slate-900/70 shadow-lg">
            ...
            <table className="w-full min-w-[1050px] border-collapse text-xs">
              <thead>
                <tr className="border-b border-slate-800 bg-slate-950/40 text-[9px] uppercase tracking-wider text-slate-400">
                  ...
                  <th className="px-3 py-2.5 text-right">Salary Expenses</th>
                  <th className="px-3 py-2.5 text-right text-emerald-400">Sales Expenses</th>
                  <th className="px-3 py-2.5 text-right text-amber-400">Capital Expenses</th>
                  <th className="px-3 py-2.5 text-right">Other Expenses</th>
                  <th className="px-3 py-2.5 text-right text-cyan-400">Diesel Expenses</th>
                  <th className="px-3 py-2.5 text-right text-emerald-400">Gross Combined Expenses</th>
                </tr>
              </thead>
                  ...
                  branches.map((branch) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <MoneyCell value={total} className="bg-emerald-950/10 font-bold text-emerald-400" />
                      </tr>
                    );
                  })
                )}
...
export default function TotalExpensesPage() {
  return (
    <Suspense fallback={<Loading text="Loading Expense Portfolio..." />}>
